          ...
          <Card className="p-4">
            ...
            <div className="flex flex-col items-center gap-3 mb-5 pb-5 border-b border-slate-100 dark:border-slate-800">
              ...
              <div className="text-center">
                ...
                <Badge
                  variant={profile?.role === 'ADMIN' ? 'warning' : 'success'}
                  className="mt-1.5"
                >
            ...
            <Card className="p-6">
              ...
              <div className="mt-5 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <p className="text-xs text-slate-400">
                  สมาชิกตั้งแต่ {profile?.createdAt?.split('T')[0] ?? '-'}
                </p>
            ...
            <Card className="p-6">
              ...
              <div className="flex flex-col gap-4">
                <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 rounded-xl">
                  ...
                  <button
                    onClick={toggleDark}
                    className={`w-12 h-6 rounded-full transition-colors relative cursor-pointer ${dark ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-600'}`}
                  >
                    <div
                      className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-transform ${dark ? 'translate-x-7' : 'translate-x-1'}`}
                    />
                  </button>
                </div>
                <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 rounded-xl">
                  ...
                  <select
                    ...
                    className="px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 cursor-pointer"
                  >
          ...
          {section === 'notifications' && (
            <Card className="p-6">
              ...
              <div className="flex flex-col gap-4">
                {[
                  {
                    label: 'ราคาหุ้นเปลี่ยนแปลง',
                    desc: 'รับแจ้งเตือนเมื่อราคาเปลี่ยนแปลงเกิน 5%',
                    default: true,
                  },
                  {
                    label: 'เป้าหมายทางการเงิน',
                    desc: 'รับแจ้งเตือนเมื่อบรรลุเป้าหมาย',
                    default: true,
                  },
                  {
                    label: 'Email สรุปรายสัปดาห์',
                    desc: 'รับสรุปพอร์ตทุกวันจันทร์',
                    default: false,
                  },
                  {
                    label: 'การแจ้งเตือนระบบ',
                    desc: 'อัปเดต, บำรุงรักษา',
                    default: true,
                  },
                ].map((n, i) => (
                  <div
...
function NotiToggle({ defaultOn }: { defaultOn: boolean }) {
  ...
  return (
    <button
      onClick={() => setOn((v) => !v)}
      className={`w-12 h-6 rounded-full transition-colors relative cursor-pointer ${on ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-600'}`}
    >
      <div
        className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-transform ${on ? 'translate-x-7' : 'translate-x-1'}`}
      />
    </button>
  );
}
